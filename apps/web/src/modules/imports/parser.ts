...
export async function parseWorkbook(buffer: Buffer, mapping: ImportColumnMapping) {
  ...
  const rows = rawRows.map((row, index) => {
    const candidate = {
      ...
      marketValue: mapping.marketValue ? row[mapping.marketValue] ?? undefined : undefined,
...
export function summarizeImport(rows: Awaited<ReturnType<typeof parseWorkbook>>) {
  const processedRows = rows.filter((row) => row.success).length;
  const errorRows = rows.length - processedRows;
  const newAssets = rows.filter((row) => row.success).map((row) => row.normalized.normalizedAssetName);

  return {
    totalRows: rows.length,
    processedRows,
    errorRows,
    distinctAssets: Array.from(new Set(newAssets)).length
  };
}
